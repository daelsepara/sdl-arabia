...
bool characterScreen(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, Story::Base *story)
{
    std::string title = "Once Upon a Time in Arabia: Adventure Sheet";
    ...
    if (window && renderer)
    {
        ...
        controls.push_back(Button(0, createHeaderButton(window, "Skills", clrWH, intYW, headerw, headerh, space), 0, 1, 0, 1, startx, starty + profileh + headerh + marginh, Control::Type::GLOSSARY));
        controls.push_back(Button(1, createHeaderButton(window, "Possessions", clrWH, intYW, headerw, headerh, space), 0, 2, 0, 2, startx, starty + profileh + 3 * headerh + 3 * marginh + 2 * boxh, Control::Type::ACTION));
        ...
        if (font)
        {
            while (!done)
            {
                ...
                {
                    putText(renderer, "Codewords", font, space, clrWH, intYW, TTF_STYLE_NORMAL, headerw, headerh, startx, starty + 2 * profileh + 4 * headerh + 4 * marginh + 2 * boxh);
                ...
                renderButtons(renderer, controls, current, intYW, space, space / 2);
...
bool glossaryScreen(SDL_Window *window, SDL_Renderer *renderer, std::vector<Skill::Base> Skills)
{
    std::string title = "Once Upon a Time in Arabia: Skills Glossary";
    ...
    if (window && renderer)
    {
        ...
        while (!quit)
        {
            ...
            renderButtons(renderer, controls, current, intYW, border_space, border_pts, (offset > 0), glossary && offset < (glossary->h - text_bounds + 2 * space));
...
bool inventoryScreen(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, Story::Base *story, std::vector<Item::Base> &Items, Control::Type mode, int limit)
{
    if (Items.size() > 0)
    {
        ...
        while (!done)
        {
            ...
            SDL_SetWindowTitle(window, "Once Upon a Time in Arabia: Possessions");
            ...
            if (!flash_message)
            {
                ...
                {
                    putText(renderer, "You are carrying too many items. Select item(s) to DROP.", font, text_space, clrWH, intYW, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
                }
                else if (mode == Control::Type::USE)
                {
                    putText(renderer, "Select an item to USE", font, text_space, clrWH, intYW, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
                }
                else if (mode == Control::Type::LOSE)
                {
                    ...
                    putText(renderer, stolen_message.c_str(), font, text_space, clrWH, intYW, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
                }
                else
                {
                    putText(renderer, "You are carrying these items", font, text_space, clrWH, intYW, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
                }
            }

            putText(renderer, "Life", font, text_space, clrWH, intYW, TTF_STYLE_NORMAL, splashw, infoh, startx, starty + text_bounds - (boxh + infoh));
...
bool takeScreen(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, std::vector<Item::Base> items, int TakeLimit, bool back_button)
{
    ...
    if (TakeLimit > 0)
    {
        ...
        while (!done)
        {
            ...
            SDL_SetWindowTitle(window, "Once Upon a Time in Arabia");
            ...
            if (!error)
            {
                ...
                putText(renderer, take_message.c_str(), font, text_space, clrWH, intYW, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
            }
            ...
            putText(renderer, "SELECTED", font, text_space, clrWH, intYW, TTF_STYLE_NORMAL, splashw, infoh, startx, starty + text_bounds - (3 * boxh + infoh));
    ...
    if (Limit > 0)
    {
        ...
        while (!done)
        {
            ...
            if (!error)
            {
                ...
                putText(renderer, lose_message.c_str(), font, text_space, clrWH, intYW, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
            }
...
void renderAdventurer(SDL_Window *window, SDL_Renderer *renderer, TTF_Font *font, Character::Base &player, Character::Gender gender)
{
    ...
    putText(renderer, name_string.c_str(), font, space, clrWH, intYW, TTF_STYLE_NORMAL, headerw, headerh, startx, starty);
    ...
    putText(renderer, "Skills", font, space, clrWH, intYW, TTF_STYLE_NORMAL, headerw, headerh, startx, starty + profileh + headerh + marginh);
    ...
    putText(renderer, "Life", font, space, clrWH, intYW, TTF_STYLE_NORMAL, headerw, headerh, startx, starty + profileh + 2 * headerh + 2 * marginh + boxh);
    ...
    putText(renderer, "Money", font, space, clrWH, intYW, TTF_STYLE_NORMAL, headerw, headerh, startx + boxw + marginw, starty + profileh + 2 * headerh + 2 * marginh + boxh);
    putText(renderer, (std::to_string(player.Money) + " dinars").c_str(), font, space, clrBK, intBE, TTF_STYLE_NORMAL, boxw, boxh, startx + boxw + marginw, starty + profileh + 3 * headerh + 2 * marginh + boxh);

    putText(renderer, "Possessions", font, space, clrWH, intYW, TTF_STYLE_NORMAL, headerw, headerh, startx, starty + profileh + 3 * headerh + 3 * marginh + 2 * boxh);
...
Character::Base customCharacter(SDL_Window *window, SDL_Renderer *renderer)
{
    std::string title = "Once Upon a Time in Arabia: Create Character";
    ...
    if (window && renderer)
    {
        ...
        while (!done)
        {
            ...
            fillRect(renderer, textwidth, text_bounds / 3 - texty, textx, 2 * (texty + text_bounds / 3), intYW);
            ...
            if (current >= 0 && current < Limit)
            {
                ...
                renderText(renderer, text, intYW, textx + text_space, 2 * (texty + text_bounds / 3) + text_space, text_bounds / 3 - texty, 0);
                ...
                else if (controls[current].Type == Control::Type::NEW)
                {
                    if (selection.size() == player.SKILLS_LIMIT)
                    {
                        ...
                        for (auto i = 0; i < selection.size(); i++)
                        {
                            ...
                            {
                                Character::CUSTOM.Items.push_back(Item::BOW);
                            }
                            else if (Skill::ALL[selection[i]].Type == Skill::Type::SWORDPLAY)
                            {
                                Character::CUSTOM.Items.push_back(Item::SWORD);
                            }
                            else if (Skill::ALL[selection[i]].Type == Skill::Type::MAGIC)
                            {
                                Character::CUSTOM.Items.push_back(Item::RING);
                            }
                        }
...
Character::Base selectCharacter(SDL_Window *window, SDL_Renderer *renderer)
{
    std::string title = "Once Upon a Time in Arabia: Select Character";
...
bool aboutScreen(SDL_Window *window, SDL_Renderer *renderer)
{
    ...
    auto splash = createImage("images/twist-of-fate.png");
    ...
    if (window && renderer && splash && text)
    {
        ...
        while (!done)
        {
            fillWindow(renderer, intYW);

            fitImage(renderer, splash, startx, starty, splashw, text_bounds);

            fillRect(renderer, text->w + 2 * text_space, text->h + 2 * text_space, startx * 2 + splashw, starty, intYW);

            renderText(renderer, text, intYW, startx * 2 + splashw + text_space, starty + text_space, SCREEN_HEIGHT * (1.0 - 2 * Margin) - 2 * text_space, 0);
...
bool saveGame(Character::Base &player, const char *overwrite)
{
    ...
    save += "/Saved Games/Once Upon a Time in Arabia";
    ...
    std::string save = std::string(homedir) + "/Documents/Saved Games/Once Upon a Time in Arabia";
    std::string path = save + "/";
...
std::vector<Button> createFilesList(SDL_Window *window, SDL_Renderer *renderer, std::vector<std::string> list, int start, int last, int limit, bool save_button)
{
    ...
    if (list.size() > 0)
    {
        for (int i = 0; i < last - start; i++)
        {
            ...
            auto button = createHeaderButton(window, game_string.c_str(), clrWH, intYW, textwidth - 3 * button_space / 2, 0.125 * SCREEN_HEIGHT, text_space);
...
Control::Type gameScreen(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, bool save_botton)
{
    ...
    if (window && renderer)
    {
        ...
        while (!done)
        {
            ...
            SDL_SetWindowTitle(window, "Once Upon a Time in Arabia: LOAD/SAVE game");
...
bool tradeScreen(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, Item::Base mine, Item::Base theirs)
{
    ...
    if (Character::VERIFY_ITEMS(player, {mine.Type}))
    {
        ...
        while (!done)
        {
            ...
            if (!error)
            {
                putText(renderer, "TRADE", font, text_space, clrWH, intYW, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
            }
...
bool shopScreen(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, Story::Base *story, Control::Type mode)
{
    ...
    if (shop.size() > 0)
    {
        ...
        for (auto i = shop.begin(); i != shop.end(); i++)
        {
            ...
            choice += " (" + std::to_string(price) + " dinars)";
        ...
        while (!done)
        {
            ...
            {
                SDL_SetWindowTitle(window, "Once Upon a Time in Arabia: Shop");
            }
            ...
            if (!error && !purchased)
            {
                if (mode == Control::Type::BUY)
                {
                    putText(renderer, "Select an item to buy", font, text_space, clrWH, intYW, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
                }
                else
                {
                    putText(renderer, "You may sell your items at prices indicated here", font, text_space, clrWH, intYW, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
                }
            }
                ...
                if (controls[current].Type == Control::Type::ACTION && !hold)
                {
                    if (current >= 0 && current < shop.size())
                    {
                        ...
                        if (mode == Control::Type::BUY)
                        {
                            ...
                            {
                                message = "You do not have enough dinars to buy that!";
...
bool mapScreen(SDL_Window *window, SDL_Renderer *renderer)
{
    ...
    auto splash = createImage("images/map-arabian-nights.png");
    ...
    {
        SDL_SetWindowTitle(window, "Map: The World of the Arabian Nights");
...
Story::Base *processChoices(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, Story::Base *story)
{
    ...
    if (renderer && story->Choices.size() > 0)
    {
        ...
        while (!done)
        {
            ...
            {
                SDL_SetWindowTitle(window, (std::string("Once Upon a Time in Arabia: ") + std::string(3 - std::to_string(std::abs(story->ID)).length(), '0') + std::to_string(std::abs(story->ID))).c_str());
            }
...
bool loseSkills(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, int limit)
{
    ...
    if (player.Skills.size() > limit)
    {
        ...
        while (!done)
        {
            ...
            putText(renderer, "SKILLS", font, text_space, clrWH, intYW, TTF_STYLE_NORMAL, splashw, infoh, startx, starty + text_bounds - (boxh + infoh));
    ...
    while (!quit)
    {
        ...
        if (window && renderer)
        {
            ...
            while (!quit)
            {
                ...
                renderButtons(renderer, controls, current, intYW, border_space, border_pts, (offset > 0), text && offset < (text->h - text_bounds + 2 * space));
...
bool mainScreen(SDL_Window *window, SDL_Renderer *renderer, int storyID)
{
    ...
    auto title = "Once Upon a Time in Arabia";
    ...
    if (window && renderer && splash && text)
    {
        ...
        while (!done)
        {
            ...
            fillRect(renderer, text->w + 2 * text_space, text->h + 2 * text_space, startx * 2 + splashw, texty, intYW);
...
int main(int argc, char **argv)
{
    ...
    createWindow(SDL_INIT_VIDEO, &window, &renderer, title, "icons/lamp.png");
